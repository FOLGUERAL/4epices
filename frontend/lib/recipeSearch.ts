...
/** Minuscules, sans accents, sans ponctuation : « Crème brûlée » → « creme brulee ». */
 ...
 */
export function searchRecipes(recipes: SwipeRecipe[], query: string, limit = 20): SwipeRecipe[] {
  const tokens = toTokens(query);
  if (tokens.length === 0) return [];

  const scored: Array<{ recipe: SwipeRecipe; score: number }> = [];
