 ...
 * L'accueil est le logo. Sur mobile, seuls Recettes et Favoris restent, et seulement quand la barre du bas est
 * absente (pages recette) : sinon elle porte déjà tous les onglets.
 */
  ...
  return (
    <>
      {TABS.filter((tab) => tab.key !== 'accueil').map(({ key, href, label }) => {
        ...
        const visibility = key === 'recettes' || key === 'favoris' ? 'hide-with-tabbar' : 'hidden md:flex';
        return (
          <Link
            key={key}
            href={href}
